...
#dd
head(dd)
dd$Fdia<-factor(dd$dia)
summary(dd)
dd$lactob
#És el mateix que dd[,"lactob"]  o bé dd[,5]
...
```
```{r}
#scatterplot(pH~dia|Ferm,dd,smooth=F,regLine=T,main="pH")
sp(pH~dia|Ferm,dd,smooth=F,regLine=T,main="pH")
abline(h=4,lt=2)
sp(pH~dia|Ferm,dd,smooth=T,regLine=F,main="pH")

sp(strep~dia|Ferm,dd,smooth=F,main="strep")
sp(lactob~dia|Ferm,dd,smooth=F,main="lactob")
```

**ii.** Gràfiques de mitjanes amb interval de confiança de l'esperança (95%) de l'estil

```{r,echo=FALSE}
with(dd,plotMeans(pH,Fdia,Ferm,error.bars="conf.int",main="pH"))
```

```{r}
with(dd,plotMeans(strep,Fdia,Ferm,error.bars="conf.int",main="strep"))
with(dd,plotMeans(lactob,Fdia,Ferm,error.bars="conf.int",main="lactob"))
```


**iii.** Taules de l'estil

```{r,echo=FALSE}
print(tabular((pH)*Ferm*(mean+sd)~Fdia,dd),justification="l")
```
```{r}
tabular((strep)*Ferm*(mean+sd)~Fdia,dd)

print(tabular((strep)*Ferm*(mean+sd)~Fdia,dd),justification="l")
print(tabular((lactob)*Ferm*(mean+sd)~Fdia,dd),justification="l")
