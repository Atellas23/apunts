...
# LAB 1: 1D kernel (ridge) regression
...
###########################
## a really nice-looking (if noisy) function
...
## may be this is a good moment to review the class notes about kernel (ridge) regression
...
## On the downside, the need of inverting a large matrix can be a big issue as N grows. Clearly we need better methods, in particular showing sparsity; note though that the kernel matrix does not depend on data dimension

## Nonetheless, an active topic of research is found in approximating
# the kernel matrix by reducing its size (Random Fourier Features, Nyström features, ...)
