...
```{r}
# Utilitzem la parametrització per defecte (té terme independent)

m<-lm(GMD~DOSI,dd)

# Gràfiques de les estimacions del model
emmip(m,~DOSI) # nomes estimació dels valors esperats
emmip(m,~DOSI,CIs=T) # Amb intervals de confiança
...
plot(m,which=1)
#plot(m,which=1,add.smooth=F)
abline(h=0,lty=2)

plot(predict(m),resid(m)) # El mateix gràfic
abline(h=0,lty=2)
...
```{r}
coef(m)
(C2<-cbind(1,contrasts(C(dd$DOSI,treatment))))
C2%*%coef(m)
(emm<-emmeans(m,~DOSI))
```

 – Comproveu que no depèn de la parametrització del model.
 
```{r}
#    Amb totes les parametritzacions 
#    sobté els mateixos resultats de l'estimació
#    de les esperances marginals (emmeans)

(emm0<-emmeans(m0,~DOSI))
(emm1<-emmeans(m1,~DOSI))
(emm2<-emmeans(m2,~DOSI))
```

• Feu les comparacions de totes les dosis entre elles. Dieu quines parelles de nivell de dosi, donen un GMD esperat diferent.

```{r}
pairs(emm) # comparació de totes les dosis entre elles
```
